(function($) {
	$(function(){
		...
		$('#casestudies-options-tab ul li:first a').colorbox({
			srollable: false,
			innerWidth: 600,
			innerHeight: 280,
			href: SITE_URL + 'admin/casestudies/categories/create_ajax',
			onComplete: function() {
				$.colorbox.resize();
				...
				$('form#categories').live('submit', function(e) {
					...
					$.ajax({
						url: SITE_URL + 'admin/casestudies/categories/create_ajax',
						type: "POST",
					        data: form_data,
						success: function(obj) {
							...
							if(obj.status == 'ok') {
								...
								$('#casestudies-options-tab li:first span').html(obj.title);
